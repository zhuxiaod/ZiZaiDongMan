...
- (void)createSelectBtn{
    ...
    [view mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.left.right.equalTo(self.maskView);
    ...
    confirmBtn.backgroundColor = ZZTSubColor;
    [confirmBtn setTitle:@"完 成" forState:UIControlStateNormal];
...
//            [self showClipImage];
            if (self.clipBlock) {
                self.clipBlock(NO,[self imageAfterClip]);
            }
            [self dismissViewControllerAnimated:YES completion:nil];
            break;
        }
